...
Example implementation::
    ...
    def _preprocess(self):
        '''
        Subclasses may override this method for variable and data initialisation before data
    ...
    def _process(self, data):
        '''
        To be overridden by a PE implementation subclass. The data input parameter may contain data
        in any format that is prepared by a PE base pattern class implementing
        :py:func:`~dispel4py.core.GenericPE.process`. By default, the data parameter contains the
        dictionary of all inputs and the return value must be the output data dictionary.
        PE base classes may override :py:func:`~dispel4py.core.GenericPE.process` as they wish to
        pre and postprocess data to and from any format.
        To produce more than one output block data can be written at any point during processing
        using the :py:func:`~dispel4py.core.GenericPE.write` method.
        '''
        None
    ...
    def process(self, inputs):
        ...
        the names of the output streams as keys.
        PE base classes may override this method to allow preprocessing of data before calling
        :py:func:`~dispel4py.core.GenericPE._process` with the prepared data on the PE implementation.
        The data returned by the implementation may then be postprocessed before it is returned to
